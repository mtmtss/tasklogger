...
## 5.7 null タスク(未割当時間の自動記録)

タスクを開始し忘れたまま PC で作業した時間を失わないため、**実行中タスクがない間のキーボード/マウス操作時間を擬似タスク「null」として自動記録**する。目的はタスク記録ミスの影響・リスクの最小化(PC を触っている時間は基本的に作業時間である、という前提)。

- ハートビートループ (15 秒毎) で監視。実行中タスクがなく、無操作時間が 60 秒以内(= 操作中)なら null 区間の記録を開始する。開始点は「最後に入力があった時刻」だが、**直前のログの end_time より前には遡らない**(実タスクとの二重計上防止)
- null 区間の終了条件と終了時刻:
  - 実タスクの開始(開始/再開/今すぐやる)→ その時点で締める(end_reason='user')
  - 無操作しきい値超過(`idle_pause_minutes` と同じ、未設定時 5 分)→ 最後に入力があった時刻で締める(end_reason='idle')
  - スリープ突入 → その時点(end_reason='sleep')。検知漏れは最後の生存確認時刻(end_reason='sleep')
  - アプリ異常終了 → 次回起動時に最後の生存確認時刻で締める(end_reason='recovery')
- ログは `work_logs` に `task_list_id = task_id = task_list_name = task_title = 'null'`、`action_type='paused'` で追記する。**60 秒未満の区間は記録しない**(ノイズ防止)
- `active_session` は使わない(状態機械の不変条件「実行中 = ユーザーが選んだタスク 1 件」を守る)。null 区間の状態は settings(`null_session_started_at` / `null_session_last_seen`)に持ち、実タスクの開始をブロックしない
- 擬似タスクは tasks テーブルに存在しないため今日リストには現れない。集計(今日ダッシュボード / アーカイブ / Sheets 同期)には「null」として現れる
- 設定 `null_tracking_enabled`(既定 true)で無効化できる
